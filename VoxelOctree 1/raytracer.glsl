...
#define firstRayDepth 200
#define secondRayDepth 50
#define shadowRayDepth 160
...
bool check(vec3 pos, int level){
	float nodeSize = float(1 << level);
	//pos /= nodeSize;
	//vec3 diff = abs(round(pos)-pos);
	vec3 diff = abs(mod(pos + (0.5*nodeSize),nodeSize)-pos);
...
void main(){
	...
	vec3 v = normalize(vec3((pixelCoords-(0.5*resolution))/resolution.y,1.));
	v *= camMat;
	...
	if (backColor != vec4(0)){
		color = 0.15*color + 0.85*backColor.xyz;
	}
	writePixel(color);
}